...
const VideoTitle = ({ title, overview, release }) => {
  const [showTrailer, setShowTrailer] = useState(false);

  const handleTrailerClick = () => {
    setShowTrailer(true);
    // Add code to play the trailer from the beginning
  };

  return (
    <div className={`row aspect-video video_title_main absolute ${showTrailer ? '' : 'bg-gradient-to-r from-black'}`}>
      <div className='col-12 col-sm-8 col-md-6 col-lg-6'>
        ...
        <div className='col-12 d-flex video_button_div'>
          {/* <div>
            <button onClick={handleTrailerClick} className='pt-2 pb-2 play_button d-flex' >
              <div>
